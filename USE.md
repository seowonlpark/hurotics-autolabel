...
`DOMAIN_NOTES.md` Section 10 first; this doc is the operational procedure, not the rationale.
...
|---|---|---|
| `periodicity` | invariant | yes |
| `grav_stab` | invariant | yes |
| `antiphase` | **rate_dependent** (delta0.17) | no never promote |
| `gyro_energy` | **rate_dependent** (delta0.50) | no never promote (failed at id=69) |

### 2. Non-redundancy

S2 already carries proxies for some anchors - e.g. `ang_LR_corr` ~= `antiphase`. (Cadence had a
proxy too, `*_angvel_dom_hz` ~= the now-dropped `gait_hz`, Section 10.8.) The champion's whole story is
*dropping* collinear features (Section 4.6; `Hip_Deg` cut at 0.991
...
(`periodicity`, `grav_stab`, and `swap_count` at `interleg_center=0`) can. Promote the
...
| `confidence` | `high` / `medium` / `low` | S2xS3 agreement tier; `low` = the abstain (disagreement) cell |
