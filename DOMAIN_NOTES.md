...
Statistics over a whole file say nothing about a state that occupies 5% of it. See Section 11 item 1:
*density needs mass.*
...
- **`B_Gyro_*` is rad/s. `L_Gyro_*` / `R_Gyro_*` is deg/s.** Same naming convention, same file.
  Any feature mixing trunk and thigh gyro without conversion is off by **57.3x**. The clean-layer
...
- **The permutation is NOT sagittality, and conflating them was a real bug [measured, 2026-07-20].**
  ...
  that has no in-file signature (Section 6.2 measured every signal-only rule for it at below chance). The
  pipeline no longer tries to pick it per file - it reads the Y plane for **every** file (Section 6.3).
...
- **Apply the r-floor PER AXIS, not per side [measured, 2026-07-20 - 11.1 one level down].**
  ...
  **25 anomalies**, nearly all of the form `{X->X, Y->Z, Z->X}` - the two real axes right, `Deg_Z`
  garbage. `Deg_Z` is the yaw-like axis that *drifts rather than oscillates* (Section 4.2), so
...
*after* cleaning gives a median slope of **0.987 / 0.984 / 0.986** for L / R / B. The 57.3x is gone -
...
gyro rate (Section 6.2). **The first yaw-derived feature must add the consumer** - writing one against
...
least one subject has only a degenerate-time-base file (Section 2.6) so it contributes no usable data. The
...
are 200 Hz. (No contradiction with Section 7's "rev2 samples at 494 Hz" - that is the *span/n* estimate on
...
- `*_angvel_LPF` is **not** `d(*_ang_LPF)/dt`. It is `LPF(Gyro)` - filtered gyro, straight from the
  ...
  The two merely *resemble* each other because `Gyro == d(Deg)/dt` (Section 4.1).
- `*_ang_LPF` is **not** always `LPF(Deg_Y)`. The source axis is **device-revision dependent.**

**[SUPERSEDED by Section 6.3 - the pipeline now ALWAYS reads the sagittal/Y plane.]** Historically the
...
the Section 4.1b permutation (X->X, Y->Z, Z->Y), so `*_angvel_LPF` is derivable from `*_ang_LPF` and the only
per-export fact was *which Deg axis the exporter treated as sagittal*. That question is retired
(Section 6.3): we read Y for every file. The genuine Section 4.1b anomalies are the two files whose *permutation*
...
ground-truth pairs: the known export axis **100%**, gait-band energy 16%, antiphasexamplitude 16%, raw
...
drift (Section 4.2). L/R antiphase is **necessary but not sufficient** - every projection of a planar leg
swing is antiphase, so it cannot discriminate. Do not re-attempt these: the fix was not to detect the
axis but to stop needing to (Section 6.3).
...
- `timestamp.m` returns only the **last** inter-sample interval: its loop overwrites `del_t` every
  ...
  `time_temp = time{i,j}` set once at `k==1`, one deltat filters the whole trial. If the final two
  timestamps ever tie, `a = 0`, the recursion becomes `y[n] = y[n-1]`, and the output **freezes flat
  for the entire trial, silently** - the Section 2.6 failure mode exactly. Checked all 44 trials:
  last-dt/median-dt in [0.90, 1.28], no zeros, so current features are sound. Fix: return the vector,
  or use `median(diff(time))`, never `diff(end)`.
- `csv2mat.m:35` does `data_table(1:5:end,:)` - **naive 5x decimation, no anti-aliasing**, precisely
  what Section 2.5 forbids. The annotated set was NOT produced with it active (paired row counts are
...
The per-export "which Deg axis is sagittal" question (Section 6.2) is **retired.** HUROTICS confirmed the
...
(Section 4.1b), and a drift check (dominant frequency ~= DC on the axis being read) is the candidate teeth
...
Y wins on both, for one reason seen twice: `Deg_Y` carries **3-4x the gait swing** of `Deg_X` on
...
driven by gait 2-4x lower amplitude than the corpus (`L_ang_ptp` 8.2deg vs 17-33deg). It could be neither
...
Y plane (Section 6.3) and reinstated; until then it is out. (An earlier draft of this section claimed rev14
...
- **The incumbent `loco` algorithm's failure profile, measured [measured, 2026-07-20 - supersedes
  ...
  recording boundaries - not absorbing bouts. This does **not** resurrect `loco`: Section 4.5 severed it as
...
- **That table is share-of-errors on DIFFERENT recordings - reconciled, and the sign flips
  ...
  pairs live). This is Section 11 exactly: comparing shares across different denominators measures the wrong
...
- **Per-file calibration, never corpus-wide.** A fixed threshold scores `walk_rec = 0.000` on rev8
  ...
  **[measured, 2026-07-21 - this is a THRESHOLD property; it does NOT transfer to the S2 ML
  ...
  on `ileg_minhalf` (Section 10.1). Built as an S2 challenger (per-file posture recentre + `angvel`
  ...
  scale is confounded by each file's walk/stand mix, and the at-rest opening (Section 10.2) cannot supply
...
- **The column count is 30, 32 or 33 depending on which question is asked. All three are right
  ...
  |---|---|---|
  | **30** | `KEEP_MEASURED` | `Time` + 27 IMU (3 sides x Deg/Gyro/Acc x XYZ) + 2 load cells |
  | 32 | 30 + the `Hip_Deg` pair | **historical - no longer produced**, see below |
  | **33** | what is on disk | 32 + `segment`, which the clean layer *adds* |

  `segment` is pipeline metadata (Section 3.1), not a raw column, which is why it appears in no `KEEP_*`
  tuple. `Label` is **not** among these: raw carries no labels (Section 0), so `KEEP_IF_PRESENT` only fires
...
- **`KEEP_EXCEPTIONS` is now EMPTY - the measured-only rule has no exceptions
  ...
  - **But not a clean function of it**, which is worse than being redundant. Affine fits give slope
    ~0.96-0.99 with a per-file offset of -75 to -88deg, and max residual **4deg to 152deg**. The
    unmodeled remainder *is* the firmware's zeroing convention - the exact firmware-version signal
    the measured-only rule exists to strip (Section 9 consequence **b**). The old entry flagged that
  ...
  - **The bridge had no far side.** Section 5.7: the open dataset is *not in the repo*. Nothing downstream
...
measures **0** and walking measures **2** on every file across a **4x amplitude range** (rev8 swings
22deg, rev2 swings 49deg), so 1 is the only integer between them.
...
**[open - protocol] rev5 breaks the assumption.** 6 of 8 rev5 files open *mid-gait* (a 36-44deg interleg
...
rev2_t1 walk-recall **0.994**, rev2_t3 **0.931** (Section 10 reported 0.989 / 0.930). The **lockbox stays
...
| `gyro_energy` | summed `angvel2` over the window | **rate_dependent** - re-derives the id=69 failure from scratch; the sum scales with sample count, so it is a claim about the *grid*, not the body (defined the failing way on purpose) |
...
artifact, not gait (Section 4.6). When that offset exceeds the swing's negative reach, `L-R` never commits
past -delta, `swap_count` reads 0, and the rule calls real, rhythmic, antiphase gait STANDING.

rev2 trials 6 and 7 carry a **+11deg interleg offset** whose per-window minimum never drops below
+3.9deg, and the raw rule calls **0.00 / 0.04** of their WALK-labeled windows walking - the two
...
confirmed it, and the remedy was measured before it was believed (Section 11.4).

**Fix: subtract the per-file rest zero (Section 10.2) before counting swaps.** `anchors.rest_offset()` takes
...
- **It does not fix the variable-amplitude failures.** After centering, the top disagreements are
  ...
  (fast/variable cadence, the Section 9/Section 10.4 stride-adaptive-window item), not an offset one. Centering
...
train+val only, per Section 10.4.
...
(Section 10.2). The "degrades gracefully on ~2/28 files" caveat was never checked. It is wrong: **6 files
(all of rev5) open mid-gait**, not at rest - rev5_t2/t6/t8 open with a **36-44deg interleg swing and
4-5 swaps**, unambiguously walking. On those the opening median is taken over gait, and a *single* 3 s
swing window is a poisoned zero: per-window medians wander **+/-4-5deg** within a file (max-min spread up
to **20deg**), because a 3 s slice catches an asymmetric chunk of a stride. rev5_t2's mid-file window
reads **8.6deg** against its true-ish **0.65deg** - off by 8x`delta`.
...
So the Section 10.5 "gait oscillates symmetrically about the offset" claim holds **at the whole-file scale,
...
1. **Trust the opening 3 s** only if the swap rule's own STANDING verdict holds on it (0 swaps on the
   locally centered span) - reusing the validated rule (Section 10), not a new stillness threshold, keeps it
...
lockbox-safe and does **not** open the seal (Section 10.4).
...
amplitude* (rev2_t6/t7: +11deg offset vs a swing barely reaching zero) - rev5's **36-44deg** swings commit
...
among all revs, so the "recordings begin at rest" external label (Section 10.2) is **not universal** - rev5
...
The swap rule counts interleg alternations in a window, and `periodicity` resolves to 1/window
(as `gait_hz` did before it was dropped, Section 10.8). A fixed 2 s window therefore fails once the stride
period nears 2 s: it holds <2 strides,
...
strides. Periodic -> window = 2xperiod (capped 6 s); non-periodic (standing has no peak above the 0.35
...
Section 10 per-window `swap_verdict`, and the disagreement ranking is now built on it: the fixed verdict
...
### 10.7 Span-grow fallback - the Section 10.6 detector still under-fires on slow/large gait **[measured, 2026-07-22]**

Section 10.6 sizes the window from `stride_period()`, but that detector *itself* fails on exactly the gait it
...
low-amplitude: the missed windows swing a **median 40deg peak-to-peak** interleg (97% >= 4deg), and **77%
...
moving-median detrend all failed to recover recall and most *hurt* stand-recall; and the fixed +/-1deg delta
is not the constraint, since the swing is 40deg not <2deg.)
...
`GROW_MIN_PTP_DEG = 8deg`, *and* the raw legs anti-correlated past `GROW_MIN_ANTIPHASE = 0.5`. The last
gate is load-bearing and is the Section 10.2/Section 11 discriminator applied at the span level: two isolated
standing weight-shifts also give `swap_count 2` over 6 s (this is precisely the stand-recall cost a
plain 6 s window pays in the Section 10.6 table), but their legs are **not** antiphase - requiring leg
...
passing only the interleg signal keeps the untouched Section 10.6 behaviour. Same caveat as Section 10.6: **lab-corpus
validated, not deployment-solved.**

### 10.8 `gait_hz` dropped - degenerate at the 2 s window, and its audit verdict was a false pass **[measured, 2026-07-22]**

`gait_hz` (cadence in Hz, the dominant gait-band FFT peak of the swing angle) was the fifth audited
anchor. At the 2 s output window the FFT resolution is 1/2 s = **0.5 Hz**, and the gait band's low edge
(0.13 Hz) is below the first resolvable bin - so the anchor **pinned at 0.5 Hz for 84% of WALK and 81%
of STAND windows**. Measured discrimination: **walk-vs-stand AUC 0.487** - literally no information
(antiphase 0.90, grav_stab 0.11 for contrast).

Worse than useless, it was **misleading in two places**. (1) The rate audit passed it "invariant,
delta=0.0000" - but that is a **degenerate pass**: both 100 Hz and 50 Hz collapse to the same 0.5 Hz bin, so
the audit cannot tell "genuinely rate-invariant body property" from "stuck at the resolution floor." It
looked like the *most* robust anchor while being the emptiest - a Section 11.1-class trap living inside the
anchor set. (2) The hypothesis agent's own prompt used "gait_hz halves -> slow-cadence tail" as a model
hypothesis, i.e. the agent could cite a phantom cadence change that the feature never had the resolution
to see.

**Removed entirely** (not demoted): `ANCHOR_NAMES`, `window_anchors`, `_dominant_hz`, the rate-audit
`ANCHOR_METRIC`/`ANCHOR_FLOOR`, the panel-3 right axis, and the agent prompt. A half-kept degenerate
feature still on the plots would still mislead. The capability worth preserving is **cadence in Hz** -
the deployment population is *defined* by slow cadence - so the path back is to recompute it on the
**Section 10.7 grown span** (up to 6 s -> 0.167 Hz resolution, enough to resolve the ~0.33 Hz slow strides of
Section 10.6), where it would carry real information *and* give the rate audit a non-degenerate quantity to
test. Not done speculatively: reintroduce it when a hypothesis actually needs cadence, not before.
`GAIT_BAND_HZ` stays (still used by `stride_period` and `_periodicity`).
...
**The outcome is real; the agent's mechanism is refuted - Section 11.4 again, now on the fusion agent.** The
agent blamed the stride-adaptive window (Section 10.6), "sized up to 6 s, bridging the pause." Measured: the
...
standing problem; it does not solve it, because the residual is **label contamination** (Section 12: the
...
physics error** (the Section 10.6/Section 12.1 slow-gait/brief-stop failures), or a transition. Measured on the
...
avoid the Section 11.2 "invented category" trap: agent proposes from the `new_class_candidate` spans ->
deterministic validation that it is a recurring cluster with *mass* (not a Section 11.1 GMM/BIC artifact) ->
`needs_human` to confirm against protocol/video. And adding a class needs richer ground truth - which
is the data lever again. Rest-relative angle removes the per-subject zeroing bias (Section 4.6) the champion
had to drop, so it is the honest feature for this, unlike raw absolute angle.

**Now built (`stages/s4_fusion/newclass.py` + `agents/s4_newclass.py`, `--phase 6`).** The
deterministic core assembles the 28 `new_class_candidate` spans (347 windows across 6 revs) into a
per-span physics-profile bundle. Their mean profile is the tell: `antiphase` **-0.30** (legs moving
*together*, not the antiphase of gait), `grav_stab` 0.29 (moving, not held), `periodicity` 0.35 (not
rhythmic) - a "moving stand" residue, consistent with the "static sit is absent, the excess is dynamic
repositioning" finding above. The agent PROPOSES named classes over these; `validate_proposal` gates
on **provenance** (every `span_ref` must overlap a real candidate span) AND **cluster mass** (>= 4
spans across >= 2 revs) before a proposal is `supported`, and stamps `needs_human: true` on every
proposal regardless. This is orthogonal to the deployed algorithm: it reads artifacts only and never
touches the S2 model, the swap rule, the fuser, or a label - so opening the lockbox first is safe.

**Lockbox evaluation of the fused model (`stages/s4_fusion/lockbox.py`).** The one honest
generalization number waits for a deliberate open (`--confirm OPEN-LOCKBOX`; it refuses otherwise -
opening spends rev8/rev13's value as an unbiased test, Section 7). It scores the **fused** model, not S2
alone: S2 fit on every non-lockbox rev predicts the sealed revs unseen, S3 verdicts are computed on
them (physics is model-free, nothing leaks by computing), fuse, score. Acting on this number and then
changing the model would invalidate it - the new-class agent does not change the model, so the two are
independent and either order is safe; only *retraining on new labels* would require a fresh test.
...
adjudicate after the fact (**late fusion**, Section 12). Tested the two anchors that are both rate-invariant
(Section 12 audit: `periodicity`/`grav_stab` invariant; `antiphase`/`gyro_energy` disqualified; `gait_hz` dropped Section 10.8)
...
| added anchor | macro-F1 | delta vs champion | note |
|---|---|---|---|
...
(Section 12/Section 12.2). `periodicity` fails the same way Section 11.2 predicts (it moves at cadence *changes*, not
...
collinearity first (the Section 4.6 discipline).** Code reverted; champion unchanged. This is Section 11.4 again in
...
S3 now exposes `rest_offset_trusted` per window (Section 10.5.1): `False` on the 6 rev5 files whose zero fell
...
walk-recall 0.786, stand-recall 0.893 either way), because a 36-44deg swing commits past +/-`delta`
regardless of a few degrees of center error - so a trust-conditioned S4 rule would fire on a
distinction that makes no difference here, adding policy surface for zero measured gain (Section 11.1, a rule
