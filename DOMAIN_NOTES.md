...
Measured from `runs/s1_census/manifest.jsonl` (91 raw files; subject = filename field 2, §5.5):
...
`runs/s2_ml/experiments.jsonl` and in `champion.json`. It is not inert documentation either:
...
- **Per-agent section injection is the named fix and nobody has scoped it [open, 2026-08-04].** The
  ...
  *contradicts* an entry; an agent shown only its own sections cannot do that for the entries it was
  ...
  `runs/*/system_prompt.txt` and review ledgers.
...
one full stride of evidence — not the rule being wrong. Verdict counts over all 6,339 windows moved
`AMBIGUOUS` 1,543 → 380.

> Those counts were rendered from `runs/s3_physics/physics.md`, which **nothing regenerates since
> 2026-08-04**: it was written by `s3_physics/run.py`, deleted with S4. The file is kept and carries a
> banner saying so — it is a snapshot, not current output, and this per-window tally exists nowhere
> else, because it came from `anchors.csv` (the 1.4 MB fusion-join table, now gone). The swap rule
> itself is unchanged and still runs: `label_audit.py` scores every trial against it, `serve.py`
> carries the identical verdict onto the serve path, and `rate_audit.py` reproduces that snapshot's
> audit table exactly (`antiphase` 0.0005, `gyro_energy` 0.4994) — the check that the rule did not
> drift when its driver was deleted and its audit re-homed.
...
tolerance 0.1, decimated 100 → 50 Hz through S1's anti-aliasing FIR (`runs/s3_physics/rate_audit.md`,
