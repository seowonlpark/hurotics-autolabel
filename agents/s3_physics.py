...
SYSTEM_PROMPT = (
    ...
    "  2. the interleg signal L_ang - R_ang with the +/-1deg swap bands, and the swap rule's "
    "stride-adaptive verdict as coloured squares along the top\n"
    "  3. the anchor timeline: antiphase, periodicity, grav_stab (bounded, one axis)\n\n"
    "The swap rule is validated and zero-parameter (DOMAIN NOTES Section 10): 0 swaps = STANDING, "
    "1 = AMBIGUOUS, >=2 = WALKING. Two corrections are already applied so you are not re-deriving "
    "them (DOMAIN NOTES Section 10.4-Section 10.6): the interleg signal is recentred on each file's rest zero "
    ...
    "the swap rule abstains while the label says WALK, the swap window grows to its 6 s cap "
    "without reaching two alternations, and periodicity stays low - a slow-cadence tail' is.\n"
