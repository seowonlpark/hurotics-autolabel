...
SYSTEM_PROMPT = (
    ...
    "  - Target the biggest error mass the report shows - the weakest class's recall, or "
    "the worst held-out rev. A change aimed at a few percent of the errors cannot move "
    "the headline metric, however sensible it sounds.\n"
    "  - Prefer a change whose mechanism you can state. 'Try more trees' is not a "
    "mechanism; 'the weakest class's errors concentrate in one held-out rev, so this is "
    "a generalization failure rather than a feature failure, so X' is.\n"
