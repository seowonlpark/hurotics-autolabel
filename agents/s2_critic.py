...
SYSTEM_PROMPT = (
    ...
    "NOTES finding, changes more than one thing at once, or targets a slice of the error "
    "too small to matter. Say which, concretely.\n"
    ...
    "either. Check `drop_features` specifically: null keeps the champion's drops, but an "
    "empty list drops NOTHING and silently re-adds them, so a hyperparameter proposal "
    "carrying `\"drop_features\": []` is two changes wearing one name. Say `revise` and name "
    "the field.\n"
    "  3. **Leverage.** Does it target the largest error mass the report actually shows - "
    "the weakest class's recall, or the worst held-out rev? A change aimed at a few "
    "percent of the errors cannot move the headline metric.\n"
...
def build_prompt(proposal: dict, ledger_rows: list[dict], report_md: str,
    ...
    return (
        ...
        "CHAMPION locoeval REPORT (per-class and per-rev):\n"
        f"{report_md}\n\n"
