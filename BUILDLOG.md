...
the persisted `champion.joblib` + `model_meta.json` are a convenience for outside use (a ready-to-run
classifier), derived *from* the spec, never a rival record of it: `train.py` loads `champion_spec.json`,
applies its drops / window / params, then refits - so the blob is reproducible, not authoritative. and
because a promotion rewrites the spec, the pipeline refits these artifacts right after a
champion/challenger cycle (`s2_refit`), so the saved model can never lag the spec it claims to be.
