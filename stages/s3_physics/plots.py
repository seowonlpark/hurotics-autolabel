...
# interleg swap signal the swap rule reads (§10), and the anchor timeline -- so the agent can
...
# (slow walking still labeled WALK, split-stance standing, §10.3). see README / PLAN S3.
...
def plot_trial(trial: Trial, out_dir: Path, spec: WindowSpec | None = None) -> Path:
    ...
    if not anchors.empty:
        ...
        axr = ax3.twinx()
        axr.plot(wt, anchors["gait_hz"], color="#ff7f0e", lw=1.0, ls=":", label="gait_hz")
        axr.set_ylabel("gait_hz (Hz)", color="#ff7f0e")
        axr.tick_params(axis="y", colors="#ff7f0e")
        lines = ax3.get_lines()[:3] + axr.get_lines()[:1]
        ax3.legend(lines, [ln.get_label() for ln in lines], loc="upper right",
